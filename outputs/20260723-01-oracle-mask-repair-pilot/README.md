# Oracle-mask Repair Pilot Outputs
...
本目录保存冻结输入 `G_good/G_bad`、50 个正式修复 SDF、逐次指标和最终汇总. `preflight.json` 和 `baselines.csv` 记录输入完整性检查及修复前基线. Smoke、日志、checkpoint、数据集副本与采样轨迹不进入 Git.
