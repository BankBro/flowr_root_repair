...
逐层筛选漏斗如下. 每一层只统计同时通过此前所有条件的结果:

| 筛选层级 | 本层淘汰 | 剩余 | 占全部 50 次 |
|---|---:|---:|---:|
| 正式采样 | - | 50 | 100% |
| 正常完成、坐标有限、可解析, 且固定坐标和冻结图字段保持 | 0 | 50 | 100% |
| 无 protein clash | 10 | 40 | 80% |
| 同时通过内部键长、键角和 internal clash 检查 | 26 | 14 | 28% |
| 额外保持严格立体化学签名 | 1 | 13 | 26% |

其中 `14/50` 是按预注册标准得到的正式成功数. 最后一层立体化学检查属于 post-hoc 分析, `13/50` 只能作为更严格的补充结果, 不用于回改本实验 gate.
